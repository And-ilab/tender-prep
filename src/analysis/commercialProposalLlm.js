...
const CP_SYSTEM_PROMPT = [
  ...
  "**Не включай служебную информацию:** в тексте КП **не перечисляй** имена файлов корпуса, ссылки на блоки вводных данных и прочие внутренние источники разделов — документ только для заказчика.",
...
/**
 * Удаляет служебный блок «источники разделов / файлы корпуса», не предназначенный для заказчика.
 * @param {string} md
 */
export function stripCommercialProposalServiceMetadata(md) {
  const lines = String(md ?? "").split("\n");
  const headerRe =
    /^\s*(?:#{1,6}\s*)?(?:\*{0,2})?(?:Разделы КП подготовлены|Источники разделов|Служебн)/i;
  ...
  if (cutFrom < 0) {
    ...
    while (i >= 0 && isCorpusTraceBulletLine(lines[i])) {
      cutFrom = i;
      i -= 1;
    }
  }
 ...
 * @param {string} line
 */
function isCorpusTraceBulletLine(line) {
  const s = String(line ?? "");
  if (!/^\s*[-•*]\s/.test(s)) return false;
  if (/Согласованная цена и условия \(менеджер\)/i.test(s)) return true;
  if (/^\s*[-•*]\s*(?:\*\*)?Раздел\s+\d+\s*\(/i.test(s) && /\.(?:pdf|docx?)\b/i.test(s)) return true;
  if (/^\s*[-•*]\s*(?:\*\*)?Приложение к коммерческому предложению\s*:/i.test(s) && /\.(?:pdf|docx?)\b/i.test(s)) {
    return true;
  }
  return false;
}
...
export async function runCommercialProposalDraftToDrive(userRootId, tenderId, opts = {}) {
  ...
  md = stripCommercialProposalServiceMetadata(md);
  if (!md) {
