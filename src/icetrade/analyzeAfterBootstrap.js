...
export async function analyzeTenderAfterBootstrap(userRootId, tenderId, opts = {}) {
  ...
  const system = [
    ...
    "- **Предложение участника (пакет):** если в КД есть «требования к форме и содержанию **предложения** участника», «ценовое предложение», «запрос ценовых предложений» — включи **отдельный документ предложения** (Коммерческое предложение / форма из приложения) в lenaCanPrepare или managerMustProvide с цитатой. **Не путай** название всего «пакета подаваемых документов» с одним документом «Предложение» по форме приложения. На шаге 1 Telegram блок **«Кроме того пакет должен содержать»** — все документы пакета **кроме** подтверждающих квалификацию; без дублей с qualificationRequirements[].confirmationDocuments.",
