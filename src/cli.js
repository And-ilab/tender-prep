...
function usage() {
  const lines = [
    ...
    "  node src/cli.js analysis …                    — анализ для КП (см. node src/cli.js analysis commercial-prompt …)",
...
async function main(argv) {
  ...
  try {
    ...
    if (cmd === "analysis") {
      const { runAnalysisCli } = await import("./analysis/cli.js");
      await runAnalysisCli(argv.slice(3));
      return;
    }
