...
## Локальный модуль (без Windmill, без npm-зависимостей)

Нужен только **Node.js 20+**. Сборка не требуется.
...
Исходники: `src/` — валидация JSON, снимок закупки, матрица соответствия. Точки расширения: позже сюда можно добавить вызов Windmill, не меняя контракты JSON.

## Структура репозитория

- `docs/` — человекочитаемый контракт и контекст (в т.ч. для ИИ-агентов).
- `src/` — реализация «Лены» (валидация JSON, снимок, матрица).
- `examples/` — примеры входа закупки и результата парсера.
