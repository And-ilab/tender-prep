...
Перенос **тяжёлой части** (сервер эмбеддингов + при желании весь `rag index`) с ноутбука на сервер, который не перезагружают.

**Telegram-бот «Лена»** целиком — [../lena-server/README.md](../lena-server/README.md) (бот + при желании embeddings и RAG на одном VPS).
