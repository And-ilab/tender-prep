...
3. **«Кроме того к подаче»** — остальные документы из Analyze **без дублей** с блоком квалификации.
...
П. **3.2 КД** («Документы и сведения…»): каждый нумерованный подпункт = отдельная строка чеклиста **на шаге 2** (и опционально «Кроме того к подаче» на шаге 1 без org/founding).
