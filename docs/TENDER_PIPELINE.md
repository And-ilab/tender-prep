...
**Ingest + verify** по `_lena/org-docs/{company}/`, `_lena/founding-docs/{company}/`, `references/`, staging `attachments/_incoming/` (строго **одно** юрлицо — gs-retail **или** finselvat):

1. **ingest** — OCR → validate (дата справки, период баланса, срок доверенности) → master-папка → **`company-docs-index.json`**;
2. **verify** — чеклист по реестру + best match;
3. **syncSubmissionPackage** — ярлыки shared masters + tender-only файлы в **`attachments/submission/`**; манифест **`notes/submission-package.json`**.
...
**После «Документы загружены»:** ingest → verify → sync → **«Проверка загрузки»** (найдено **valid** / ещё не загружено + «загрузить») + ссылка **«Комплект для печати»** → `submission/` → **«Проверить снова»** / **«Продолжить»** (только «Продолжить» → §5 Commercial terms).
