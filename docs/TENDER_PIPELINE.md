...
3. **«Кроме того пакет должен содержать»** — остальные документы пакета из Analyze **без дублей** с блоком квалификации; при наличии **Предложения** — пояснение, что это отдельный обязательный документ по форме КД (не путать с названием всего пакета).
