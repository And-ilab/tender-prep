...
1. **Требования к квалификации** — массив `qualificationRequirements[]` из Analyze: краткие саммари с цифрами и условиями из КД (не эталонные названия документов).
2. **Полный «К подаче»** — все документы из матрицы Analyze (включая `storage: org|founding`: справка банка, баланс, ОФР, учредительные и т.д.) с подсказками по **дате/периоду** из КД (`bankReferenceDateRule`, `balanceSheetPeriodRule`, `incomeStatementPeriodRule`). **Наличие на Drive не проверяем** — только после выбора **ГС Ритейл** / **Финсельват**.
...
| Квалификация | — | Отдельный блок `qualificationRequirements` — саммари с цифрами; **не** «Референс-лист» |
...
П. **3.2 КД** («Документы и сведения…»): каждый нумерованный подпункт = отдельная строка чеклиста **на шаге 2** (и опционально «К подаче» на шаге 1 без org/founding).
